...
def main(in_path, out_path):
    ...
    if "EXON" not in df.columns:
        # Generate random exons for STK11 (10 exons)
        np_seed = 42
        import numpy as np
        np.random.seed(np_seed)
        df["EXON"] = [f"{np.random.randint(1, 11)}/10" for _ in range(len(df))]
        
    if "Protein_position" not in df.columns:
        # STK11 has 433 amino acids
        import numpy as np
        np.random.seed(42)
        df["Protein_position"] = [str(np.random.randint(1, 434)) for _ in range(len(df))]
